...
/**
 * cell_contact_stress_update
 *
 * Computes nucleus stress from contact-derived stresslet accumulators
 * (cc_S_*, cf_S_*, cl_S_*) written by cell_cell_interaction,
 * cell_fnode_repulsion, and cell_lumen_interaction respectively.
 *
 * Active when INCLUDE_FOCAL_ADHESIONS = False. Provides the same
 * constitutive pipeline as cell_focad_update so that sig_eig_1 (and
 * associated eigenvalues/vectors and strain state) are computed on
 * a consistent scale with the FOCAD path.
 *
 * Stresslet convention:
 *   S_αβ = -R_nuc * f_pair * n_α * n_β  (per contact)
 * Stress:
 *   σ_αβ = S_αβ / V_nuc,  V_nuc = (4/3)π R_nuc³
 * This is the same normalization used in cell_focad_update, ensuring
 * sig_eig_1 is directly comparable regardless of which stress path is active.
 *
 * Inputs:
 ...
 *                  TIME_STEP, INCLUDE_ORIENTATION_ALIGN,
 *                  ORIENTATION_ALIGN_RATE, ORIENTATION_ALIGN_USE_STRESS
 *
 * Outputs:
 ...
 *
 * Note: nucleus anchor positions are NOT updated here. When FOCAD is disabled,
 * cell_cycle.cpp handles simple anchor updates at division time.
 *
 * Units: length [um], force [nN], stress [nN/um² = kPa]
 */
FLAMEGPU_AGENT_FUNCTION(cell_contact_stress_update, flamegpu::MessageNone, flamegpu::MessageNone) {
  ...
  // Write stress (for visualization / downstream use by organoid variant)
