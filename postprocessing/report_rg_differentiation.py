...
extracts per-cell RG-specific scalars and vectors, and produces:
  ...
  - A ``rg_differentiation_<tag>.png`` 4-panel figure:
      ...
      Panel 1 — Cell type composition (iPSC / NPC / RG counts)
...
-----------------------
  ...
  1 → NPC   (Pax6+ neural progenitor cell)
  2 → RG    (radial glia)
...
_CELL_TYPE_LABELS = {0: "iPSC", 1: "NPC", 2: "RG"}
...
def _aggregate(snap: dict, step: int, dt_s: float) -> dict:
    ...
    row["f_npc"] = row["n_npc"] / n if n > 0 else 0.0
    ...
    row["commit_npc_mean"]  = _safe_mean(cl, npc_mask)
...
def main() -> None:
    ...
    for vtk in vtk_files:
        ...
        print(
            ...
            f"  iPSC={row['n_ipsc']:3d}  NPC={row['n_npc']:3d}  RG={row['n_rg']:3d}"
