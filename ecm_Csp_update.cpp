...
FLAMEGPU_AGENT_FUNCTION(ecm_Csp_update, flamegpu::MessageNone, flamegpu::MessageNone) {
  ...
  const uint32_t ECM_POPULATION_SIZE = 68921; // WARNING: this variable must be hard coded to have the same value as the one defined in the main python function.

  auto C_SP_MACRO = FLAMEGPU->environment.getMacroProperty<float, N_SPECIES, ECM_POPULATION_SIZE>("C_SP_MACRO");

  //if (agent_grid_lin_id % 1000 == 0) {
  //  printf("[DEBUG ecm_Csp 0] gid=%-5d  C_SP_MACRO[0]=%.3f\n",
  //         agent_grid_lin_id, (float)C_SP_MACRO[0][agent_grid_lin_id]);
  //  printf("[DEBUG ecm_Csp 1] gid=%-5d  C_SP_MACRO[1]=%.3f\n",
  //         agent_grid_lin_id, (float)C_SP_MACRO[1][agent_grid_lin_id]);
  //}
